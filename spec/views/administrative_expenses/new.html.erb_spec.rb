...
describe "/administrative_expenses/new.html.erb" do
  include AdministrativeExpensesHelper
  
  before(:each) do
    assigns[:administrative_expense] = stub_model(AdministrativeExpense,
      :new_record? => true,
      :copies => 1,
      :copies_expense => 9.99,
  ...
  it "renders new administrative_expense form" do
    render
    
    response.should have_tag("form[action=?][method=post]", administrative_expenses_path) do
