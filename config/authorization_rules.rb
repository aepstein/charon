authorization do
  ...
  role :user do
    ...
    has_permission_on [ :activity_reports ], :to => :manage do
      if_permitted_to :fund_request, :organization
    end

    has_permission_on [ :fund_requests ], :to => :show do
      if_permitted_to :fund_request
    end
    has_permission_on [ :fund_requests ], :to => :show do
    ...
    has_permission_on [ :fund_requests ], :to => :fund_request, :join_by => :and do
      if_permitted_to :fund_request, :organization
      if_attribute :fund_source => { :framework_id => is_in { object.organization.frameworks( Edition::PERSPECTIVES.first ).map(&:id) } }
      if_attribute :fund_source => { :framework_id => is_in { user.framework_ids( Edition::PERSPECTIVES.first, object.organization.roles.ids_for_user( user ) ) } }
      if_attribute :fund_source => { :open_at => lte { Time.zone.now } }
    end
    has_permission_on [ :fund_requests ], :to => :review, :join_by => :and do
      if_permitted_to :review, :fund_source
      if_attribute :organization_id => is_not_in { user.organization_ids }
      if_attribute :fund_source => { :framework_id => is_in { object.fund_source.organization.frameworks( Edition::PERSPECTIVES.last ).map(&:id) } }
      if_attribute :fund_source => { :framework_id => is_in { user.framework_ids( Edition::PERSPECTIVES.last, object.fund_source.organization.roles.ids_for_user( user ) ) } }
    end
    ...
    has_permission_on [ :fund_requests ], :to => :manage, :join_by => :and do
      ...
      if_attribute :fund_source => { :closed_at => gte { Time.zone.now } }
    end
    ...
    has_permission_on [ :fund_items ], :to => :fund_request do
      if_permitted_to :fund_request, :fund_request
    end
    ...
    has_permission_on [ :fund_items ], :to => :update, :join_by => :and do
      if_permitted_to :review, :fund_request
      if_attribute :fund_request => { :status => is_in { %w( accepted ) } }
    end
    ...
    has_permission_on [ :fund_editions ], :to => :manage, :join_by => :and do
      ...
      if_attribute :fund_item => { :fund_request => { :status => is_in { %w( accepted ) } } }
    end
    has_permission_on [ :fund_editions ], :to => :manage, :join_by => :and do
    ...
    has_permission_on [ :fund_editions ], :to => :show, :join_by => :and do
      if_permitted_to :show, :fund_item
      if_attribute :fund_item => { :fund_request => { :status => is { 'released' } } }
    end
    ...
    has_permission_on [ :activity_accounts ], :to => :show do
      if_permitted_to :fund_request, :university_account
