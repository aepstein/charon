authorization do
  ...
  role :user do
    ...
    has_permission_on [ :approvals ], to: [ :new, :create ], join_by: :and do
      ...
      if_attribute approvable_type: is { 'Agreement' },
        approvable_id: is_not_in { user.approved_agreement_ids }
    end
    has_permission_on [ :approvals ], to: [ :new, :create ], join_by: :and do
      if_permitted_to :approve, :approvable
      if_attribute user_id: is { user.id }
      if_attribute approvable_type: is { 'FundRequest' },
        approvable_id: is_not_in { user.approved_fund_request_ids }
    end
    ...
    has_permission_on [ :fund_grants ], to: :request, join_by: :and do
      ...
      if_attribute requestor_memberships: {
        user: is { user },
...
#        frameworks: contains { object.fund_source.reviewer_framework }
    ...
    has_permission_on [ :fund_requests ], to: :approve, join_by: :and do
      if_permitted_to :request_framework, :fund_grant
      ...
      if_attribute fund_grant: { fund_source: { open_at: lt { Time.zone.now },
        ...
        requestor_memberships: {
          user: is { user },
          frameworks: contains { object.fund_grant.fund_source.submission_framework }
        }
      }
    end
    ...
    has_permission_on [ :organizations ], to: [ :request, :update ] do
      if_attribute memberships: {
        user_id: is { user.id },
        active: is { true },
        role: { name: is_in { Role::REQUESTOR } } }
    end
