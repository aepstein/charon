...
function add_child_item(el, depth) {
  try {
    ...
    for (i = 0; i < depth; i++) {
      id_children_str += "_new_request_items_";
      name_children_str += "[new_request_items][]";
    }

    var html = '\n<label for="request_new_request_items__request_amount">Request amount</label>\n';
    html += '<input id="request' + id_children_str + '_request_amount" name="request' + name_children_str + '[request_amount]" size="30" type="text">\n';
    html += '<input id="request' + id_children_str + '_should_destroy" name="request' + name_children_str + '[should_destroy]" value="0" type="hidden" class="should_destroy">\n';
